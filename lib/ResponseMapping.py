...
class ResponseMapping:
    ...
    def __init__(self, x_dim: int, batch_size: int, cost_fn_batched: Callable = Commons.quad_cost_cvxpy_batched,
    ...
    def create_optimization_problem(self) -> None:
        ...
        """
        self.X = cp.Variable((self.batch_size, self.x_dim))
        self.R = cp.Parameter((self.batch_size, self.x_dim))
        self.w = cp.Parameter(self.x_dim)
        self.b = cp.Parameter(1)
        self.f_der = cp.Parameter((self.batch_size, self.x_dim))
        ...
        CCP_target_train = self.CCP_target_batched(self.X, self.R, self.w, self.b, self.f_der, self.train_slope,
                                                   self.cost_const_kwargs)
        CCP_target_eval = self.CCP_target_batched(self.X, self.R, self.w, self.b, self.f_der, self.eval_slope,
                                                  self.cost_const_kwargs)
    ...
    def create_differential_optimization_problem(self) -> None:
        ...
        r = cp.Parameter(self.x_dim, value=np.random.randn(self.x_dim))
        w = cp.Parameter(self.x_dim, value=np.random.randn(self.x_dim))
        b = cp.Parameter(1, value=np.random.randn(1))
        f_der = cp.Parameter(self.x_dim, value=np.random.randn(self.x_dim))

        CCP_target_grad = self.CCP_target_not_batched(x, r, w, b, f_der, self.train_slope, self.cost_const_kwargs)
        ...
        self.CCP_layer = CvxpyLayer(CCP_problem_grad, parameters=[r, w, b, f_der], variables=[x])

    def solve_optimization_problem(self, X: tensor, w: tensor, b: tensor, **kwargs) -> tensor:
        """
        ...
        while diff > self.diff_threshold and iteration < self.iteration_cap:
            ...
            self.f_der.value = Commons.f_der_numpy(approximation_X, w_np, b_np, slope)
    ...
    def solve_differential_optimization_problem(self, X: tensor, w: tensor, b: tensor, X_opt: tensor, **kwargs) -> tensor:
        """
        ...
        """
        f_der = Commons.f_der_torch(X_opt, w, b, self.train_slope)
        return self.CCP_layer(X, w, b, f_der)[0]

    def optimize_X(self, X: tensor, w: tensor, b: tensor, requires_grad: bool = True, **kwargs) -> tensor:
        """
        ...
        X_opt_batches = [self.solve_optimization_problem(X_batch, w, b, **kwargs) for X_batch in X_batches]
        ...
        if requires_grad:
            X_opt = self.solve_differential_optimization_problem(X, w, b, X_opt, **kwargs)

        return X_opt

    def CCP_target_batched(self, X, R, w, b, f_der, slope, cost_kwargs):
        """
        ...
        """
        gain = cp.diag(X @ f_der.T) - Commons.g_batch(X, w, b, slope)
        cost = self.cost_batched(X, R, **cost_kwargs)
        return gain - cost

    def CCP_target_not_batched(self, x, r, w, b, f_der, slope, cost_kwargs):
        """
        ...
        """
        gain = x @ f_der - Commons.g(x, w, b, slope)
        cost = self.cost_not_batched(x, r, **cost_kwargs)
        return gain - cost

    def split_and_pad(self, X: tensor) -> tensor:
        """
        ...
        :param X: The matrix to split to batches.
        :return: The matrix X, split to batches of size batch_size.
        """
        X_batches = list(X.split(self.batch_size))
        X_batches[-1] = self.pad_to_batch_size(X_batches[-1])
        return X_batches

    def pad_to_batch_size(self, X: tensor) -> tensor:
        """
        ...
        :param X: The matrix to pad to self.batch_size.
        :return: The padded matrix.
        """
        if len(X) == self.batch_size:
            return X
        elif len(X) > self.batch_size:
            error_message = f"The passed matrix is of size {X.shape}, while trying to pad to {self.batch_size}."
            raise ValueError(error_message)

        pad_amount = self.batch_size - len(X)
        return F.pad(X, (0, 0, 0, pad_amount), "constant", 0)
